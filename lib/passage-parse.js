...
function chineseToNumber(str) {
  ...
  // DayByWord 簡寫：二一=21、二三=23（無「十」字）
  if (/^[一二三四五六七八九零\u25cb]+$/u.test(rest)) {
    if (rest.length === 1) return CN_DIGIT[rest] ?? 1;
    let compact = 0;
    for (const ch of rest) {
      const d = CN_DIGIT[ch];
      if (d === undefined) return 0;
      compact = compact * 10 + d;
    }
    return compact || 1;
  }

  return n || CN_DIGIT[rest] || 0;
}
